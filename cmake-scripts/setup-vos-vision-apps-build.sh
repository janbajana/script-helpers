...
SOURCE_PROJECT="vos-vision-apps"
...
if [ "$BUILD_SYSTEM" = "Android" ]; then
    ...
    # -DSXR_HOME="E:/AndroidStudio/latencytests-hermosa/sxr-3.0.2"
    ...
    cmake -B"${BUILD_LOCATION}" -H"${SOURCE_LOCATION}" -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
        ...
        -DCMAKE_PREFIX_PATH="${BUILD_FOLDER}/install-Android-Release" \
        -DCMAKE_FIND_ROOT_PATH="${BUILD_FOLDER}/install-Android-Release" \
        ...
        -DVOSVisionCoreCpp_DIR="${BUILD_FOLDER}/vos-vision-build-${BUILD_SYSTEM}-${BUILD_TYPE}-${BUILD_ARCH}" \
        ...
        -DVOSAppKit_DIR="${BUILD_FOLDER}/vos-app-kit-build-${BUILD_SYSTEM}-${BUILD_TYPE}-${BUILD_ARCH}/cmake" \
        -DENABLE_LEGACY_RUNTIME=OFF \
        -DENABLE_ANDROID_RUNTIME=ON \
        -DWITH_MODEL_VIEWER=ON \
        -DWITH_CUBE_SAMPLE=OFF \
        -DSXR_HOME="${SXR_HOME}"
